...
func NewStreamChatHandler(req *restful.Request, resp *restful.Response) {
	...
	//鉴权
	if !my_utils.IsAuth(sessionID, req, resp) {
		return
	}
	...
	my_service.CreateAndSaveMessage(
		userMsgId,
		sessionID,
		parentId,
		"user",
	...
	stream := my_service.GlobalStreamManager.GetOrCreateStream(sessionID, assistantMsgId, userMsgId, reqBody.QueryInfo.Query, false)

	go my_service.StreamChat(stream, reqBody, sessionID, resp)
...
func ResumeStreamChatHandler(req *restful.Request, resp *restful.Response) {
	...
	var reqBody my_requests.ResumeStreamChatReq
	if err := req.ReadEntity(&reqBody); err != nil {
		resp.WriteError(http.StatusBadRequest, err)
		return
	}
	...
	// 获取或创建流状态
	stream := my_service.GlobalStreamManager.GetOrCreateStream(sessionID, reqBody.MessageID, "", "", true)
	if stream == nil {
...
func dealStreamChat(stream *my_service.StreamState, resume bool, req *restful.Request, resp *restful.Response) {
	...
	chunkChan := my_service.GlobalStreamManager.RegisterClient(stream, clientID)
	if chunkChan == nil {
...
func BreakStreamChatHandler(req *restful.Request, resp *restful.Response) {
	...
	var reqBody my_requests.BreakStreamChatReq
	if err := req.ReadEntity(&reqBody); err != nil {
		resp.WriteError(http.StatusBadRequest, err)
		return
	}
	...
	// 中断流
	exists, err := my_service.GlobalStreamManager.BreakStream(sessionID, reqBody.MessageID)
	if !exists {
	...
	resp.WriteHeaderAndEntity(http.StatusOK, my_response.BreakStreamChatResponse{Success: exists, Message: "break stream success"})
}
