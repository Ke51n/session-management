...
func StreamChatHandler(req *restful.Request, resp *restful.Response) {
	...
	// 获取或创建流状态
	stream := my_manager.GlobalStreamManager.GetOrCreateStream(sessionID, "", reqBody.QueryInfo.Query)
	...
	chunkChan := my_manager.GlobalStreamManager.RegisterClient(stream, clientID)
	if chunkChan == nil {
	...
	// 确保在函数退出（连接断开）时注销客户端
	defer my_manager.GlobalStreamManager.UnregisterClient(stream.SessionID, clientID)
...
func ResumeStreamChatHandler(req *restful.Request, resp *restful.Response) {
	...
	// 获取或创建流状态
	stream := my_manager.GlobalStreamManager.GetOrCreateStream(sessionID, reqBody.MessageID, "")
	clientID := req.HeaderParameter("X-Client-ID")
	if clientID == "" {
		clientID = uuid.NewString()
	}
	...
	// 告知客户端连接已建立，并返回会话和消息ID信息
	// is_resume 字段指示当前是否为续传模式
	my_service.SendSSE(writer, flusher, "resume", map[string]any{
		"message_id": stream.MessageID,
		"session_id": stream.SessionID,
